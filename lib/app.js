...
var app = function (config) {
    ...
    this.imgurDownload = function(rawURL) {
        if (rawURL.indexOf('imgur') > -1 && helper.getFilename(rawURL).indexOf('.') === -1) {
            var code = helper.getFilename(rawURL);
            var opts = imgur.generateOpts(code);
            return new Promise(function(resolve, reject) {
                core.getData(opts)
                .then(function (data) {
                    core.downloadImage(data.data.link, this.config.image_folder, helper.getFilename(data.data.link))
                    .then(function (path) {
                        resolve(path);
                    })
                    .catch(function (err) {
                ...
                .catch(function (err, res,body) {
                    ...
                    .then(function (data) {
                        ...
                        core.downloadImage(link, this.config.image_folder, helper.getFilename(link))
                        .then(function (path) {
                            resolve(path);
                        })
                        .catch(function (err) {
    ...
    this.handleURL = function (url) {
        return new Promise(function(resolve, reject) {
            ...
            if (url.indexOf('imgur') > -1 && helper.getFilename(url).indexOf('.') === -1) {
                ...
                .then(function (path) {
                    os.setWallpaper(this.config.image_folder, path);
                ...
                core.downloadImage(url, this.config.image_folder, helper.getFilename(url))
                .then(function (path) {
                    os.setWallpaper(this.config.image_folder, path);
                    resolve();
                })
    ...
    this.restart = function () {
        var restartTime = helper.pickRandomNumber(this.config.max_time, this.config.min_time);
        // var restartTime = helper.pickRandomNumber(30000, 5000);
        console.log("Current time:", moment().format("h:mm:ssa"));
